...
contains
  ...
  subroutine before
    ...
    a = mrandn( n, 0.1_wp ) + diag( n, 1.0_wp, real( n, wp ) )
    c = 0.0_wp
    a_tl = mrandn( n, 1.0_wp )
    c_ad = mrandn( n, 1.0_wp )
  ...
  subroutine test_ad
    ...
    a_ad = 0.0_wp
    call op_ad( n, a, a_ad, c, c_ad )
    call op_tl( n, a, a_tl, c, c_tl )

    call assert_equals( "test_ad", 0.0_wp, fp( n, c_tl, c_ad ) - fp( n, a_tl, a_ad ), EPS )
...
end module inverse_mod_test


!> @brief Runs the testsuite.
program main
  use inverse_mod_test, only: run_testsuite
