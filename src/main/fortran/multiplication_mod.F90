...
contains
  ...
  subroutine op_ad( n, a, a_ad, b, b_ad, c, c_ad )
    ...
    c = matmul( a, b )
    a_ad = a_ad + matmul( c_ad, transpose( b ) )
    b_ad = b_ad + matmul( transpose( a ), c_ad )
  ...
  subroutine op_tl( n, a, a_tl, b, b_tl, c, c_tl )
    ...
    c = matmul( a, b )
    c_tl = matmul( a_tl, b ) + matmul( a, b_tl )
