...
!> @brief Second quadratic forn: C = B^T A B
module first_quadratic_form_mod
...
contains
  ...
  subroutine op_ad( n, a, a_ad, b, b_ad, c, c_ad )
    ...
    c = matmul( transpose( b ), matmul( a, b ) )
    h1 = matmul( b, c_ad )
    h2 = matmul( b, transpose( c_ad ) )
    b_ad = b_ad + matmul( a, h2 ) + matmul( transpose( a ), h1 )
    a_ad = a_ad + matmul( h1, transpose( b ) )
  ...
  subroutine op_tl( n, a, a_tl, b, b_tl, c, c_tl )
    ...
    c = matmul( transpose( b ), matmul( a, b ) )
    h1 = matmul( transpose( b_tl ), matmul( a, b) )
    h2 = matmul( transpose( b ), matmul( a, b_tl) )
    h3 = matmul( transpose( b ), matmul( a_tl, b ) )
    c_tl = h1 + h2 + h3
...
end module first_quadratic_form_mod