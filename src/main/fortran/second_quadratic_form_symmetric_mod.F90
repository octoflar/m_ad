...
!> @brief Second quadratic forn: C = B^T A-1 B where A is symmetric

module second_quadratic_form_symmetric_mod
...
contains
  ...
  subroutine op_ad( n, a, a_ad, b, b_ad, c, c_ad )
    ...
    call inv( n, a, a_inv )
    h = matmul( a_inv, b )
    c = matmul( transpose( b ), h )
    b_ad = b_ad + matmul( h, transpose( c_ad ) + c_ad ) 
    a_ad = a_ad - matmul( h, matmul( c_ad , transpose( h ) ) )
  ...
  subroutine op_tl( n, a, a_tl, b, b_tl, c, c_tl )
    ...
    h = matmul( a_inv, b )
    c = matmul( transpose( b ), h )
    c_tl = matmul( transpose( b_tl ), h ) + matmul( transpose( h ), b_tl - matmul( a_tl, h ) ) 
...
end module second_quadratic_form_symmetric_mod
